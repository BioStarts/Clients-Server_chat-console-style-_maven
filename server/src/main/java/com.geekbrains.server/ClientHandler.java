...
public class ClientHandler {
    ...
    public ClientHandler(Server server, Socket socket, ExecutorService executorService) throws IOException {
        ...
        executorService.execute(() -> {
            ...
            try {
                while (!checkAuth()) ;//проверяем аутентификацию
                while (readMessage()) ;//считываем сообщения пока тру (пока не отключался клиент например)
...
//        }
    ...
    private boolean checkAuth() throws IOException {
        ...
        if (msg.startsWith("/auth ")) {
            ...
            String nick = server.getAuthService().getNicknameByLoginAndPassword(tokens[1], tokens[2]);
            if (nick != null && !server.isNickBusy(nick)) {
                sendMsg("/authok " + nick + " " + tokens[1]);//передаем ник и логин на клиента после авторизации
                /* ниже реализация отправки последних десяти сообщений в чат толькочто авторизованного клиента именно из общего лог файла броадкастных сообщений
                if (FileStoryChat.lastMsg().size() > 10) {
                    for (int i = 10; i > 0; i--) {
                        sendMsg(FileStoryChat.lastMsg().get(FileStoryChat.lastMsg().size() - i));
                    }
    ...
    private boolean readMessage() throws IOException {
        String msg = in.readUTF();
        if (!msg.startsWith("/")) { // говорим что если сообщение не начинается с / - значит оно не сллужебное и печатаем всем, если нет - там уже обрабатываем, в зависимости от команды
            server.broadcastMsg(nickname + ": " + msg);
            //FileStoryChat.newMessageAllClientsLog(nickname + ": " + msg);// отправляем общее сообщение в историю - лог файл
            System.out.println(msg);
            return true;
